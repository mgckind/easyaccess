...
class Import(object):
    def do_import(self, line):
        """
        Use to import modules to call functions inline query,
        similar to the import module in python.
        ...
        try:
            ...
            func_list = [f for f in getmembers(globals()[modname]) if (
                isfunction(f[1]) and hasattr(f[1], 'in_easyaccess'))]
            if len(func_list) > 0:
    ...
    def do_help_function(self, line):
        ...
        if function.lower() == 'all':
            ...
            for k in fun_utils.ea_func_dictionary.keys():
                print('    ' + k)
            return
        if function not in fun_utils.ea_func_dictionary.keys():
            print(colored("\nFunction {0} is not loaded, please import module "
                          "(check help import for more info)\n".format(function), "red"))
            return
        else:
            print("\nHelp for {0}:\n".format(function))
    ...
    def complete_help_function(self, text, line, start_index, end_index):
        if text:
            return [function for function in fun_utils.ea_func_dictionary.keys()
                    if function.startswith(text)]
        else:
